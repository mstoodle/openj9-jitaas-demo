...
if [[ "$1" == "--help" ]]; then
	echo 'Usage: startAll.sh [<hotspot container size> <openj9 container size> <openj9 nojit container size>]'
	exit 1
fi

if [[ $# == 3 ]]; then
	HOTSPOT_CONTAINER_SIZE=$1
	OPENJ9_CONTAINER_SIZE=$2
	OPENJ9_NOJIT_CONTAINER_SIZE=$3
else
	HOTSPOT_CONTAINER_SIZE=256m
	OPENJ9_CONTAINER_SIZE=256m
	OPENJ9_NOJIT_CONTAINER_SIZE=256m
fi
echo Using container sizes:
echo "    Hotspot      : $HOTSPOT_CONTAINER_SIZE"
...
echo Starting server_nojit2
./run_server_nojit2.sh $OPENJ9_NOJIT_CONTAINER_SIZE

echo Starting server_openj9
./run_server_openj9.sh $OPENJ9_CONTAINER_SIZE

echo Starting server_hotspot
./run_server_hotspot.sh $HOTSPOT_CONTAINER_SIZE
